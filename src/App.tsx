...
// Calls the real Flask backend: POST /ask -> returns job_id, then poll
// GET /status/<job_id> until status === "done" or "error".
//
// The `topic` parameter carries the user's intent (name + description); we
// concatenate them into a natural-language question that the planner can
// classify and route. For more specificity, callers can pass selectedText
// via the optional `extraContext` argument.
...
const NoteEditor: React.FC<{
  ...
  const animate = async (topic: AnimatableTopic) => {
    ...
    const [animResult, sections] = await Promise.all([
      MOCK_generateAnimation(topic, toolbar.selectedText || undefined),
